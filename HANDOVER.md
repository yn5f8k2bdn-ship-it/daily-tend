...
- **Flutter app — auth, onboarding persistence, and daily check-in submit all working end-to-end (2026-05-17)**
  ...
  - **Coach screen — code complete, smoke test blocked by Supabase pause (2026-06-08):** [`app/lib/data/coach_repository.dart`](app/lib/data/coach_repository.dart) exposes `CoachRepository` (`sendMessage` calls `generate_coach_reply` via `functions.invoke` with the user's JWT, `fetchHistory` reads `coach_messages`) + a `CoachNotifier` AsyncNotifier that owns the live transcript with optimistic append + rollback on error. [`app/lib/screens/coach/coach_screen.dart`](app/lib/screens/coach/coach_screen.dart) rewritten end-to-end: chat bubbles, action chips on the latest coach reply, starter chips on empty state, send-on-enter, auto-scroll. Analyzed clean. Could not smoke-test because the free-tier Supabase project paused (see "Free-tier pause-and-wipe" gotcha below).
...
0. **Re-add the two Edge Function secrets** before testing the Coach screen — `OPENAI_API_KEY` (from password manager, or generate a new one at https://platform.openai.com → API keys) and `OPENAI_MODEL` (e.g. `gpt-5-mini`). Go to Supabase → Edge Functions → Secrets. Project itself is restored and both functions (`generate_coach_reply` + `nightly_summariser`) were redeployed at end of 2026-06-08 session; only secrets remain.
1. **Smoke-test the Coach screen on the emulator** — code is committed and analysed clean; just needs the backend back up. Send a starter prompt, expect a coach bubble + action chips, then verify a row landed in `coach_messages` in the Supabase Table Editor.
2. **Free-tier keep-alive or upgrade decision** — either (a) GitHub Action that pings the project weekly to reset the inactivity timer, (b) upgrade to Supabase Pro ($25/mo, never pauses), or (c) accept the redeploy dance every couple of weeks.
3. **Drift offline cache for check-ins** — locked tech decision but not yet implemented. Add `drift` + codegen, schema mirroring `check_ins`, sync queue, connectivity detection, write-through; replaces direct Supabase write in [`CheckinRepository`](app/lib/data/checkin_repository.dart). ~1.5-2 hours of focused work.
4. **Capture stress/energy/sleep as first check-in** — those three onboarding screens currently just navigate. They should either write a check-in on completion of onboarding, or be removed in favour of just prompting the first proper check-in via the FAB.
5. **Schedule `nightly_summariser`** via `pg_cron` — needs a SQL snippet plus the service-role key in Supabase Vault per [`supabase/README.md`](supabase/README.md).
6. **Rules engine for daily plan** — derive `day_type` + three zone-action strings from today's check-in. Home dashboard's "Three small things" are still static placeholders.
7. **Continue phases 4–7** of [`docs/plan.md`](docs/plan.md) — progress charts, settings polish, reminders/widget, Health Connect.
8. **Re-enable Supabase email confirmation + configure Site URL + redirect templates** before any production sign-ups (currently OFF for dev convenience).
9. **Production signing config** — release keystore + register its SHA-1 with a new Google Cloud Android OAuth client before Play Store submission. Debug keystore is dev-only.
...
- **Supabase free-tier pause wipes Edge Functions + secrets, but NOT the database.** Confirmed 2026-06-08: project paused after ~3 weeks of no activity. Restoring it via the dashboard's "Restore project" button brings the DB back intact (profiles, check-ins, coach_messages all preserved) but the Edge Functions list is empty and so is Edge Functions → Secrets. Recovery: redeploy `generate_coach_reply` + `nightly_summariser` via "Via Editor", then re-add `OPENAI_API_KEY` + `OPENAI_MODEL`. Symptom in the app: `Failed host lookup: 'fpgadotiobvesybkeemz.supabase.co'` (project paused = hostname stops resolving) followed by `Coach didn't answer: FunctionException(status: 404, code: NOT_FOUND)` once restored but before redeploy.
